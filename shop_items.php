...
<head>
    ...
    <title>Shop Items - Unturned Portal</title>
                ...
                <div class="row text-center">
                    <div class="col-lg-12">
                        <div class="main-point" style="padding-top: 20px; padding-bottom: 20px;">
                            <h2>Shop Items List | PVP Server</h2>
                            ...
                            <div style="margin-bottom: 30px; text-align: center;">
                                <form method="GET" action="" style="display: inline-block;">
                                    ...
                                    <button type="submit" style="padding: 10px 20px; background: #007bff; color: white; border: none; border-radius: 5px; margin-left: 10px; cursor: pointer;">Search</button>
                                    <?php if(isset($_GET['search']) && !empty($_GET['search'])): ?>
                                        <a href="shop_items.php" style="padding: 10px 20px; background: #6c757d; color: white; text-decoration: none; border-radius: 5px; margin-left: 10px;">Clear</a>
                                    <?php endif; ?>
                                </form>
                            </div>
                            ...
                            <table class="table table-stripped table-hover datatab">
                                <thead>
                                    <tr>
                                        ...
                                        <th>
                                            <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'itemname', 'order' => (isset($_GET['sort']) && $_GET['sort'] == 'itemname' && isset($_GET['order']) && $_GET['order'] == 'asc') ? 'desc' : 'asc'])); ?>" 
                                               style="color: #333; text-decoration: none; display: flex; align-items: center; justify-content: space-between;">
                                                Item
                                                <span style="font-size: 12px;">
                                                    <?php if(isset($_GET['sort']) && $_GET['sort'] == 'itemname'): ?>
                                                        <?php echo (isset($_GET['order']) && $_GET['order'] == 'desc') ? '▼' : '▲'; ?>
                                                    <?php else: ?>
                                                        ↕
                                                    <?php endif; ?>
                                        ...
                                        <th>
                                            <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'buyback', 'order' => (isset($_GET['sort']) && $_GET['sort'] == 'buyback' && isset($_GET['order']) && $_GET['order'] == 'asc') ? 'desc' : 'asc'])); ?>" 
                                               style="color: #333; text-decoration: none; display: flex; align-items: center; justify-content: space-between;">
                                                Sell
                                                <span style="font-size: 12px;">
                                                    <?php if(isset($_GET['sort']) && $_GET['sort'] == 'buyback'): ?>
                                                        <?php echo (isset($_GET['order']) && $_GET['order'] == 'desc') ? '▼' : '▲'; ?>
                                                    <?php else: ?>
                                                        ↕
                                                    <?php endif; ?>
                                ...
                                <tbody>
                                    ...
                                    if (!empty($search)) {
                                        ...
                                        $whereClause = "WHERE itemname LIKE '%$search%' OR id LIKE '%$search%'";
                                    }
                                    ...
                                    $allowedSorts = ['id', 'itemname', 'cost', 'buyback'];
                                    ...
                                    $query = mysqli_query($conn, "SELECT * FROM uconomyitemshop $whereClause ORDER BY $sort $order");
                                    ...
                                    if ($rowCount == 0) {
                                        echo '<tr><td colspan="4" style="text-align: center; padding: 20px; color: #666;">No items found matching your search.</td></tr>';
                                    }
                                    ?>
                                </tbody>
                            </table>          
